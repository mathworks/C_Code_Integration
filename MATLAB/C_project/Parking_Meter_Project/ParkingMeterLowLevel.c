...
/* Named Constants */
#define b_TEST_MODE                    (1)

/* Variable Definitions */
int TEST_MODE;

/* Function Declarations */
static int TestMode_check_get_data(int max_data);
static void TestMode_display_value(void);
static void TestMode_give_back_coin(void);
static void TestMode_write_register(void);
static int amount_inserted_coins(unsigned char *old_reg_value);
static void display_cents_value(int cents_value);
static void display_digit(unsigned char display_ID, int digit_value, double
  dot_segment);
static int div_nde_s32_floor(int numerator, int denominator);
static void give_back_coin(int amount);
static void handle_ticket_purchase(void);
static bool is_able_to_give_back_any_coins(void);
static int return_coin_value(int amount_to_return, int coin_value);
...
static void TestMode_write_register(void)
{
  ...
  int i;
  bool exitg1;
  static const unsigned short uv0[7] = { 40960U, 40961U, 40962U, 40963U, 40976U,
    40977U, 40978U };
  ...
  while (!is_right_value) {
    ...
    /*  Force the string format definition in the generated C code for scanf_s */
    format = "%x";
    scanf_s(format, &register_text, 1);
    register_address = (unsigned short)register_text;

    /*  Check if the provided address is pointing to an existing register  */
    i = 0;
    exitg1 = false;
    while ((!exitg1) && (i < 7)) {
      if ((unsigned short)register_text == uv0[i]) {
        is_right_value = true;
        exitg1 = true;
      } else {
        i++;
...
/*
 * Arguments    : unsigned char *old_reg_value
 * Return Type  : int
 */
static int amount_inserted_coins(unsigned char *old_reg_value)
{
  ...
  if (new_reg_value != *old_reg_value) {
    ...
    if (((new_reg_value & 1) != 0) && ((*old_reg_value & 1) == 0)) {
      new_amount = 10;
    } else if (((new_reg_value & 2) != 0) && ((*old_reg_value & 2) == 0)) {
      new_amount = 20;
    } else if (((new_reg_value & 4) != 0) && ((*old_reg_value & 4) == 0)) {
      new_amount = 50;
    } else if (((new_reg_value & 8) != 0) && ((*old_reg_value & 8) == 0)) {
      new_amount = 100;
    } else {
      if (((new_reg_value & 16) != 0) && ((*old_reg_value & 16) == 0)) {
        new_amount = 200;
      }
    }
...
static void display_cents_value(int cents_value)
{
  ...
  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
  if (cents_value == 45) {
    display_digit(1U, 45, 0.0);
    display_digit(2U, 45, 0.0);
    display_digit(3U, 45, 0.0);

    /*  Compute the right value to display on the parking meter */
  } else if ((cents_value >= 0) && (cents_value <= 999)) {
    /*  Compute the cent */
    digit_value = cents_value - div_nde_s32_floor(cents_value, 10) * 10;
    display_digit(1U, digit_value, 0.0);

    /*  Compute the tens of cents */
    b_remainder = (int)rt_roundd((double)(cents_value - digit_value) / 10.0);
    ...
    display_digit(3U, x - div_nde_s32_floor(x, 10) * 10, 1.0);
  } else {
...
static void handle_ticket_purchase(void)
{
  ...
  do {
    ...
    if (inserted_amount < 120) {
      ...
      display_cents_value(120 - inserted_amount);
      ...
      if ((b_a & 2) != 0) {
        ...
        remaining_amount = return_coin_value(inserted_amount, 100);
        remaining_amount = return_coin_value(remaining_amount, 50);
        remaining_amount = return_coin_value(remaining_amount, 20);
        return_coin_value(remaining_amount, 10);
        exitg1 = 1;
      } else {
        /*  Update the amount of money to pay if a coin has been inserted */
        temp_amount = amount_inserted_coins(&register_value);
      ...
      /*  Set the selected bit to '1' and write the new value to the register */
      b_register_value = (unsigned char)(a | 16);
      ...
      /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      /*  Return as much as possible higher coin's values at first */
      b_remaining_amount = return_coin_value(inserted_amount - 120, 100);
      b_remaining_amount = return_coin_value(b_remaining_amount, 50);
      b_remaining_amount = return_coin_value(b_remaining_amount, 20);
      return_coin_value(b_remaining_amount, 10);
      exitg1 = 1;
    }
  } while (exitg1 == 0);
}
...
static bool is_able_to_give_back_any_coins(void)
{
  ...
  /*  Select the specific coin's quantity to check */
  return ((a & 1) != 0) && ((b_a & 2) != 0) && ((c_a & 4) != 0) && ((d_a & 8) !=
    0);
}

/*
 * Arguments    : int amount_to_return
 *                int coin_value
 * Return Type  : int
 */
static int return_coin_value(int amount_to_return, int coin_value)
...
static void standard_operation_mode(void)
{
  ...
  do {
    ...
    do {
      ...
      can_give_coins_back = is_able_to_give_back_any_coins();
    ...
    if (master_switch_off) {
      ...
      /*  OUT   : - */
      /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
      write_register(40963, (unsigned char)(c_a | 128));
...
static void test_mode(void)
{
  ...
  do {
    ...
    if (choice != 0) {
      ...
      /*  Get and check the menu entry provided at the console interface */
      choice = TestMode_check_get_data(8);
      printf("\n");
      fflush(stdout);
      switch (choice) {
       case 0:
       ...
       case 3:
        ...
        do {
          ...
          /*  Force the string format definition in the generated C code for scanf_s */
          format = "%d%c";
          nb_input_types = scanf_s(format, &state, &terminator, 1);
       ...
       case 7:
        ...
        do {
          ...
          /*  Force the string format definition in the generated C code for scanf_s */
          c_format = "%d%c";
          c_nb_input_types = scanf_s(c_format, &b_data, &c_terminator, 1);
        ...
        for (i = 0; i <= i0; i++) {
          ...
          /*  OUT   : - register_value: value stored in the register as an unsigned 8 bit */
          /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
          varargin_1 = read_register(40978);
          ...
          /*  OUT   : - register_value: value stored in the register as an unsigned 8 bit */
          /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
          b_varargin_1 = read_register(40977);
          ...
          /*  OUT   : - register_value: value stored in the register as an unsigned 8 bit */
          /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
          c_varargin_1 = read_register(40976);
          printf("Pushed buttons : %x\n", c_varargin_1);
          fflush(stdout);
          printf("Elapsed time   : %3.1f\n", (float)rt_roundd((double)((1 + i) *
                   100) / 1000.0));
          fflush(stdout);
