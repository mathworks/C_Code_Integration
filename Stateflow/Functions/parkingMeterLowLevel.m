...
function [] = standard_operation_mode(const_obj)
  ...
  chart = Scheduler('obj',StateMachineParameters,'REFRESHING_TICK',0);
  % Infinite loop
  while (1)
    % Run the scheduler in Stateflow
    step(chart);
    % Process ISRs when raised
    while any(chart.obj.ISR)
      % Identified raised ISRs
      isr_idx = int8(find(chart.obj.ISR == true));
      for k=1:1:length(isr_idx)
        switch isr_idx(k)
          case (1)
            % Update the values of the 7 segment displays
            display_cents_value(const_obj,chart.obj.display_value);
          case (2)
            % Update the coins flap status
            set_coin_flap_position(const_obj,chart.obj.flap_open);
          case (3)
          ...
          case (4)
            ...
            return_coins(const_obj,chart.obj.amount_to_return);
            chart.obj.amount_to_return = uint16(0);
          case (5)
            % Check if the buttons have changed their states
            chart.obj.green_pressed = is_green_button_pressed(const_obj);
            chart.obj.red_pressed   = is_red_button_pressed(const_obj);
            chart.obj.master_switch = is_master_switch_on(const_obj);
          ...
          case (6)
            ...
            [chart.obj.temp_amount,chart.obj.register_value] = ...
              amount_inserted_coins(const_obj,chart.obj.register_value);
            chart.obj.inserted_amount = chart.obj.inserted_amount+...
                                        chart.obj.temp_amount;
          case (7)
            % Terminate the execution of the application
            master_switch_off = true;
            break;
          otherwise
            % An unknown interrupt has been detected
            fprintf('Error: Non existant ISR #%d.\n',isr_idx(k));
        end
        % Reset the ISR once it has been served
        chart.obj.ISR(isr_idx(k)) = false;
      end
